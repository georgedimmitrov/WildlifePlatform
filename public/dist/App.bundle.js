...
function autocomplete(input, latInput, lngInput) {
  ...
  // enter on habitat field -> don't submit the form
...
function ajaxDelete(e) {
  ...
  if (!deleteBtn || deleteBtn && deleteBtn.classList.contains('delete__button')) {
    deleteBtn = (0, _bling.$)('.delete__button');
  }

  if (deleteBtn) {
    if (confirm('Are you sure you want to delete this record?')) {
      _axios2.default.post('/api/remove/' + deleteBtn.getAttribute('data-id')).then(function (res) {
...
// ajax request when 'hearting' an animal, so page refresh is not needed
...
function loadPlaces(map) {
  var lat = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 40.2;
  var lng = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : -66.7;

  _axios2.default.get('/api/animals/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    var places = res.data;
    if (!places.length) {
    ...
    // click on a marker -> show the details of that place
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        var html = '\n          <div class="popup">\n            <a href="/animal/' + this.place.slug + '">\n              <img src="/uploads/' + (this.place.photo || 'animal.png') + '" alt="' + this.place.animal + '" />\n              <p>' + this.place.animal + ' - ' + this.place.location.habitat + '</p>\n            </a>\n          </div>\n        ';
        infoWindow.setContent(html);
        infoWindow.open(map, this);
      });
    });

    // zoom the map just enough to fit all the markers
...
function searchResultsHTML(animals) {
  return animals.map(function (animal) {
    return '\n      <a href="/animal/' + animal.slug + '" class="search__result">\n        <strong>' + animal.animal + '</strong>\n      </a>\n    ';
  }).join('');
}
...
(0, _autocomplete2.default)((0, _bling.$)('#habitat'), (0, _bling.$)('#lat'), (0, _bling.$)('#lng'));
...
var deleteAnimalBtn = (0, _bling.$)('.delete__button');

if (deleteAnimalBtn) {
  deleteAnimalBtn.on('click', _delete2.default);
}
